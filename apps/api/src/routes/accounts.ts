...
/** Public account shape — never leaks the OBS password or the client secret. */
...
// List all OBS accounts for an org
accounts.get("/api/orgs/:orgId/accounts", requireAuth, resolveOrg("owner"), async (c) => {
  const orgId = c.req.param("orgId");
  const rows = await listAccounts(orgId);
  return c.json(rows.map(publicAccount));
});

// Create an OBS account (obs-websocket connection settings)
accounts.post("/api/orgs/:orgId/accounts", requireAuth, resolveOrg("owner"), async (c) => {
  const orgId = c.req.param("orgId");
  const parsed = createAccountBody.safeParse(await c.req.json());
  if (!parsed.success) {
    return c.json({ error: parsed.error.issues[0].message }, 400);
  }

  const id = crypto.randomUUID();
  await createAccount({ id, orgId, ...parsed.data });
  const account = await getAccount(id);
  return c.json(publicAccount(account!), 201);
});

// Update OBS connection settings — pushed live to the agent if connected
accounts.patch("/api/orgs/:orgId/accounts/:accountId", requireAuth, resolveOrg("owner"), async (c) => {
  const orgId = c.req.param("orgId");
  ...
  const parsed = updateAccountBody.safeParse(await c.req.json());
  if (!parsed.success) {
    return c.json({ error: parsed.error.issues[0].message }, 400);
  }
  ...
  if (updated) pushObsConfig(updated);
  return c.json(publicAccount(updated!));
});

// Remove an OBS account
accounts.delete("/api/orgs/:orgId/accounts/:accountId", requireAuth, resolveOrg("owner"), async (c) => {
  ...
  closeAgent(account.id);
  await deleteAccount(account.id);

  return c.json({ ok: true });
});

// SSE stream for connection status changes (hub + browser agent / OBS state)
accounts.get("/api/orgs/:orgId/accounts/events", requireAuth, resolveOrg("owner"), async (c) => {
  ...
  const snapshot = (accountId: string) => ({
    accountId,
    hub: { connected: isAccountConnected(accountId), reason: getDisconnectReason(accountId) },
    obs: getObsState(accountId),
  });
  ...
  return c.body(
    new ReadableStream({
      start(controller) {
        ...
        // Send initial state
        send(JSON.stringify({ type: "init", accounts: rows.map((a) => snapshot(a.id)) }));

        const onStatus = (event: { accountId: string; orgId: string }) => {
          if (event.orgId !== orgId) return;
          send(JSON.stringify({ type: "status", ...snapshot(event.accountId) }));
        };
...
// Enroll manually with a token pasted from Rawtoh (advanced)
...
// Forget the signing key (module can no longer connect)
