...
import agentRoutes, { websocket } from "./routes/agent";
import authRoutes from "./routes/auth";
import { sessionStorage } from "./session-storage";

const PORT = parseInt(process.env.PORT || "10700", 10);
...
// Start hub connections (accounts that already have credentials)
startAllConnections();

export default {
  port: PORT,
  hostname: "0.0.0.0",
  idleTimeout: 0, // long-lived SSE + agent WebSocket
  fetch: app.fetch,
  websocket,
};

console.log(`[obs-api] Listening on http://0.0.0.0:${PORT}`);
